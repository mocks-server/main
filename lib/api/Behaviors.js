...
class BehaviorsApi {
  ...
  getCurrent(req, res) {
    tracer.verbose(`Sending current behavior | ${req.id}`);
    res.status(200);
  ...
  putCurrent(req, res) {
    ...
    tracer.verbose(`Changing current behavior to "${newCurrent}" | ${req.id}`);
    this._behaviors.current = newCurrent;
    this.getCurrent(req, res);
  }

  getCollection(req, res) {
    tracer.verbose(`Sending behaviors collection | ${req.id}`);
    res.status(200);
