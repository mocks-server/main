...
const Api = require("../api/Api"); // TODO, remove dependency

class ServerEmitter extends EventEmitter {}

// TODO, create a new Core Class, which should orchestrate Server, Behaviors, Files, Settings, Options, etc, and expose only desired methods

class Server {
  constructor(mocksFolder, options = {}) {
    this._mocksFolder = this.resolveFolder(mocksFolder);
    ...
    // TODO, deprecate feature option compatibility
  ...
  init() {
    ...
    const currentBehavior =
      (this._behaviors && this._behaviors.currentName) ||
    ...
    this._express.use("/mocks", new Api(this._behaviors, this._settings).router);
