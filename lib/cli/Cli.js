...
class Cli {
  ...
  init() {
    ...
    this._cli = new inquirer.Inquirer(
      this._questions,
      this.header.bind(this) // TODO, deprecate quit method
    );
    this._logLevel = this._settings.get("log");
    this._filesLoaded = this._filesLoaded.bind(this);
  }

  start() {
    this.__stopListeningFilesLoad = this._core.onLoadFiles(this._filesLoaded);
    this._silentTraces();
  ...
  async _displayLogs() {
    ...
    await this._cli.logsMode(() => {
      this._tracer.set("log", this._logLevel);
    });
    this._silentTraces();
    return this._displayMainMenu();
  }
